 ...
 * @author liu 2015-1-5
 */
public abstract class LunarUtil {
    public static final Map<DayOfWeek, String> WEEK_MAP = new HashMap<>();
    ...
    static {
        ...
        WEEK_MAP.put(DayOfWeek.SATURDAY, "星期六");
        WEEK_MAP.put(DayOfWeek.SUNDAY, "星期日");
    }

    public static final SimpleDateFormat LUNAR_FORMAT = new SimpleDateFormat("yyyy-MM-dd");
     ...
     * 阳历日期计算起点
     */
    private final static LocalDate START_DATE = LocalDate.parse("1900-01-30");
     ...
     * 阴历转换为阳历
     *
     * @param date          阴历日期,格式YYYYMMDD
     * @param leapMonthFlag 是否为闰月
     * @return 阳历日期, 格式：YYYYMMDD
     * @throws Exception
     * @author liu 2015-1-5
     */
    public static LocalDate lunarToSolar(LocalDate date, boolean leapMonthFlag) throws Exception {
        int lunarYear = date.getYear();
        int lunarMonth = date.getMonth().getValue();
        ...
        LocalDate myDate = START_DATE.plusDays(offset);
        return myDate;
    }
     ...
     * @author liu 2015-1-5
     */
    public static String solarToLunar(LocalDate myDate) throws Exception {
        int i;
        int temp = 0;
        int lunarYear;
        int lunarMonth; //农历月份
